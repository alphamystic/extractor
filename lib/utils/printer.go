...
/*
  * This package just prints given text or error message, will probably implement an error logger later on
*/
import (
  "os"
  "fmt"
...
func Interactor(text string,admin bool){
  ...
  if admin {
    ...
    e.Printf("INTERACTING WITH CLIENT: \n")
  }
  interactor := color.New(color.FgCyan,color.Bold).PrintfFunc()
  interactor("   |--  " + text + ":> ")
}

type ShortPTSB func(string,string)
var PTSB ShortPTSB = PTSBWrapper

func PTSBWrapper(colorName string, text string) {
  PrintTextInASpecificColorInBold(colorName, text)
}
